...
export function registerCollabEventsRoutes(app: Express) {
  ...
  // PATCH /api/collab-events/:eventId/participants/:participantId/role - Update participant role
  app.patch("/api/collab-events/:eventId/participants/:participantId/role", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const userId = (req as AuthenticatedRequest).user.id;
      const { eventId, participantId } = req.params;
      const { role } = req.body;
      ...
      if (!["owner", "participant"].includes(role)) {
        return res.status(400).json({ error: "Invalid role. Must be 'owner' or 'participant'" });
      }
      ...
      if (event.ownerId !== userId) {
        return res.status(403).json({ error: "Only the event owner can change participant roles" });
      }
      
      // Get participant details before update
      const participantBefore = await storage.getParticipant(participantId);
      if (!participantBefore) {
        return res.status(404).json({ error: "Participant not found" });
      }
      
      // Verify that the participant belongs to this event
      if (participantBefore.eventId !== eventId) {
        return res.status(400).json({ error: "Participant does not belong to this event" });
      }
      
      // Update the role
      const participant = await storage.updateParticipantRole(participantId, role);
      ...
      await storage.createAuditLog({
        userId,
        action: "UPDATE_PARTICIPANT_ROLE",
        resource: "collaborative_event_participant",
        resourceId: participantId,
        details: {
          eventId,
          participantId,
          participantName: participant.name || participant.email,
          oldRole: participantBefore.role,
          newRole: role,
          changedBy: userName,
